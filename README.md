...
FLIR/Teledyne Spinnaker 카메라를 ROS 2 Humble 또는 ROS 1 Noetic에서 쓰기 위한 워크스페이스다.
...
환경 스크립트는 기본으로 Humble을 먼저 찾고, Humble이 없으면 Noetic을 찾는다.
명시적으로 고르려면 `FLIR_ROS_DISTRO`를 지정한다.
...
- ROS 2 Humble 또는 ROS 1 Noetic source
- `/opt/spinnaker`가 있으면 `SPINNAKER_ROOT` 설정
- ROS 2면 `install/setup.bash`, ROS 1이면 `devel/setup.bash` source
- `FLIR_ROS_WS` export
...
Noetic 경로는 현재 카메라 스트리밍과 undistort viewer를 지원한다.
